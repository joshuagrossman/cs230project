...
def test():
	# Do the same conversion for each file in WTC2
	# fileSearchKeys = [(PRELUDE_WAV_NAME_TEMPLATE % str(x)) for x in range(1, constants.NUM_PIECES_TO_TEST // 2 + 1)] + \
	# 				 [(FUGUE_WAV_NAME_TEMPLATE % str(x)) for x in range(1, constants.NUM_PIECES_TO_TEST // 2 + 1)]
	...
	# TODO: do we really want a different file for each slice?
